    ...
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 mt-3">
                <a href="{{ route('appointment.show', ['month' => $previousMonth, 'year' => $previousYear]) }}" class="btn btn-outline-secondary">&lt; Mois précédent</a>
                <h4>{{ $currentMonth->translatedFormat('F Y') }}</h4>
                <a href="{{ route('appointment.show', ['month' => $nextMonth, 'year' => $nextYear]) }}" class="btn btn-outline-secondary">Mois suivant &gt;</a>
            </div>
            ...
            <div class="row row-cols-7 g-3">
                @foreach ($daysOfMonth as $day)
                    <div class="col text-center">
                        <a href="{{ route('appointment.show', ['month' => $currentMonth->month, 'year' => $currentMonth->year, 'day' => $day]) }}" class="btn btn-light w-100 @if($day == request('day')) active @endif">
                            {{ $day }}
                        </a>
                    </div>
                @endforeach
            ...
            <!-- Affichage des rendez-vous disponibles pour le jour sélectionné -->
            @if (request('day'))
                <div class="mt-5">
                    <h5>Rendez-vous pour le {{ request('day') }}/{{ $currentMonth->month }}/{{ $currentMonth->year }}</h5>
                                ...
                                <tbody>
                                    @foreach ($appointments as $appointment)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="#" class="btn btn-success"><i class="bi bi-check-circle"></i> Réserver</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
